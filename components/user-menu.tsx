...
export const UserMenu = ({ user }: Props) => {
  return (
    <Dropdown showArrow placement='bottom-end' radius='sm'>
      <DropdownTrigger>
        <Avatar className='cursor-pointer' name={user?.name || ''} src={user?.image || ''} />
      </DropdownTrigger>
      <DropdownMenu aria-label='user menu' disabledKeys={['avatar']}>
            ...
            <DropdownItem key='avatar' isReadOnly className='opacity-100'>
              <div className='flex gap-2'>
                ...
                <div>
                  ...
                  <p className='text-tiny capitalize'>{user.name?.toLocaleLowerCase()}</p>
                </div>
              </div>
            </DropdownItem>
          </DropdownSection>
        ...
        <DropdownSection aria-label='private and public'>
          {user ? (
            <DropdownItem key='profile' startContent={<UserIcon />} onPress={handleSignOut}>
              Perfil
            </DropdownItem>
          ) : null}
          <DropdownItem key='theme' isReadOnly className='opacity-100' endContent={<ThemeSwitch />}>
            Tema
          </DropdownItem>
          <DropdownItem
            key='logout'
            color={user ? 'danger' : 'default'}
            endContent={user ? <LogOutIcon size={20} /> : <LogInIcon size={20} />}
